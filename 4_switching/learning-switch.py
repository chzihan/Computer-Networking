...
class LearningSwitch(DynamicPolicy):
    ...
    def print_switch_tables(self):
        ...
        for switch in self.forwarding_table.keys():
            for address in self.forwarding_table[switch].keys():
                write_forwarding_entry(switch, forwarding_table[switch][address], address)
        finish_log()
    ...
    def learn_route(self, pkt):
        ...
        self.forwarding_table[str( get_switch(pkt) )][get_src_mac(pkt)] = get_inport(pkt)
    ...
    def build_policy(self):
        ...
        for switch in self.forward_table.keys():
            for address in self.forward_table[switch].keys():
                if new_policy == None:
                    new_policy = (match(switch=int(switch), dstmac=address) >> 
                                  fwd(self.forward_table[switch][address]))
                else:
                    new_policy += (match(switch=int(switch), dstmac=address) >> 
                                   fwd(self.forward_table[switch][address]))
                
                if not_flood_pkts == None:
                    not_flood_pkts = (match(switch=int(switch), dstmac=address))
                else:
                    not_flood_pkts |= (match(switch=int(switch), dstmac=address))
